...
class CredentialScanner(BaseScanner):
    ...
    def _scan_file_for_credentials(self, file_path: Path, rootfs: Path) -> List[Vulnerability]:
        ...
        skip_binaries = {
            "passwd", "login", "su", "sudo", "chpasswd", "newusers",
            "ssh", "sshd", "dropbear", "telnetd", "ftpd",
            "busybox", "ash", "bash", "sh", "csh", "zsh",
            "useradd", "usermod", "userdel", "groupadd",
            "cmp", "diff", "grep", "awk", "sed", "cat", "ls",
            "free", "top", "ps", "mount", "umount", "df",
            "openssl", "curl", "wget",
        }
    ...
    def _is_false_positive(self, credential: str, cred_type: str) -> bool:
        ...
        help_text_patterns = [
            ...
            "retype", "re-type", "reenter", "re-enter", "confirm",
            "incorrect", "invalid", "wrong", "error", "failed",
            "enter", "input", "type", "provide", "specify",
            "new", "old", "current", "previous",
            "passwords", "must", "should", "cannot", "required",
            "minimum", "maximum", "length", "character",
            "match", "matches", "matching", "mismatch",
            "expire", "expired", "expiration", "expires",
            "change", "changed", "changing", "reset",
            "authentication", "authenticate", "authenticating",
            "login", "logon", "logout", "logged",
            "please", "press", "click", "select",
            "prompt", "dialog", "message", "warning",
            "success", "successful", "successfully",
            "denied", "rejected", "refused", "forbidden",
            "empty", "blank", "missing", "null", "none",
            "usage", "help", "info", "hint", "option",
            "encryption", "encrypted", "decrypt",
            "verify", "verified", "verification",
            "enabled", "disabled", "enable", "disable",
            "set", "unset", "clear", "cleared",
        ]
        ...
        if cred_type == "password":
            ...
            if any(credential.startswith(prefix) for prefix in [
                "Password", "Passwords", "Re", "New", "Old", "Enter", "Type",
                "Invalid", "Incorrect", "Wrong", "Bad", "Your", "The", "A ", "An ",
            ]):
                return True
        ...
        generic_tokens = [
            ...
            "default", "value", "name", "path", "file", "dir",

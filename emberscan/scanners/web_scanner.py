...
from ..core.models import (
    FirmwareInfo, ScanResult, ScanStatus, Vulnerability,
...
class WebScanner(BaseScanner):
    ...
    def _discover_endpoints(self, base_url: str) -> List[Dict]:
        ...
        common_paths = [
            ...
            '/admin/', '/admin/login.html', '/cgi-bin/',
            '/userRpm/', '/HNAP1/', '/goform/', '/apply.cgi',
            '/setup.cgi', '/system.html', '/status.html',
            '/config.html', '/network.html', '/wireless.html',
            '/security.html', '/firmware.html', '/backup.html',
            '/diagnostic.html', '/ping.html', '/traceroute.html',
            '/debug.html', '/test.html', '/info.html',
            '/sysinfo.cgi', '/syscmd.cgi', '/command.cgi',
            '/api/', '/api/v1/', '/jsonrpc',
    ...
    def _test_default_credentials(self, base_url: str) -> List[Vulnerability]:
        ...
        login_paths = [
            '/login.html', '/login.htm', '/admin/login.html',
            '/cgi-bin/login.cgi', '/userRpm/LoginRpm.htm',
            '/index.html', '/', '/admin/',
        ]
                    ...
                    try:
                        ...
                        if endpoint.get('method') == 'POST':
                            response = self.session.post(
                                endpoint['url'], data=data, timeout=10
                            )
                        else:
                            response = self.session.get(
                                endpoint['url'], params=data, timeout=10
                            )
    ...
    def _check_command_injection_response(self, response: str, payload: str) -> bool:
        ...
        indicators = [
            ...
            'bytes from', 'icmp_seq',  # ping output
        ]
        ...
        for endpoint in endpoints:
            ...
            for param_name, values in params.items():
                ...
                for payload in PATH_TRAVERSAL_PAYLOADS[:3]:
                    try:
                        test_url = url.replace(f"{param_name}={values[0]}", f"{param_name}={payload}")
    ...
    def _check_information_disclosure(self, base_url: str) -> List[Vulnerability]:
        ...
        sensitive_paths = [
            '/config.bin', '/config.xml', '/backup.tar.gz',
            '/etc/passwd', '/proc/version', '/proc/cpuinfo',
            '/.htpasswd', '/.htaccess', '/web.config',
            '/phpinfo.php', '/info.php', '/test.php',
            '/server-status', '/server-info',
            '/debug', '/debug.html', '/debug.cgi',
        ]

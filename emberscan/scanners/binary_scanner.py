...
# Backdoor indicators
BACKDOOR_STRINGS = [
    'backdoor', 'rootshell', '/bin/sh -i',
    'telnetd -l /bin/sh', 'nc -l -p',
    'busybox telnetd', 'dropbear -R',
    'shell_backdoor', 'hidden_shell',
]
...
class BinaryScanner(BaseScanner):
    ...
    def _check_suid_binaries(self, rootfs: Path) -> List[Vulnerability]:
        ...
        dangerous_suid = {
            'nmap', 'python', 'perl', 'ruby', 'php',
            'vim', 'vi', 'nano', 'less', 'more',
            'find', 'awk', 'sed', 'tar', 'zip',
            'bash', 'sh', 'dash', 'zsh', 'csh',
            'nc', 'netcat', 'socat', 'wget', 'curl',
        }
    ...
    def _search_backdoors(self, rootfs: Path) -> List[Vulnerability]:
        ...
        for item in rootfs.rglob('*'):
            ...
            # Check text files
            if item.suffix.lower() in search_extensions or item.name in ['rcS', 'rc.local', 'inittab']:
                try:
    ...
    def _analyze_key_binaries(self, rootfs: Path) -> List[Vulnerability]:
        ...
        key_binaries = [
            'busybox', 'httpd', 'lighttpd', 'uhttpd',
            'dropbear', 'telnetd', 'nvram', 'cfg_manager',
        ]

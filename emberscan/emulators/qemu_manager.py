...
from ..core.models import (
    FirmwareInfo, EmulationState, Architecture, Endianness
)
...
class QEMUManager:
    ...
    def _calculate_image_size(self, rootfs_path: Path) -> int:
        ...
        total_size = sum(
            f.stat().st_size for f in rootfs_path.rglob('*') if f.is_file()
        )
    ...
    def _build_qemu_command(
        ...
        cmd = [
            profile.binary,
            '-M', profile.machine,
            '-m', str(self.config.qemu.memory),
            '-kernel', str(kernel_path),
            '-drive', f'file={rootfs_image},format=raw,if=virtio',
            '-append', f'root=/dev/vda console={profile.console} rw',
            '-nographic',
        ]
        ...
        if self.config.qemu.network_mode == 'user':
            ...
            cmd.extend([
                '-netdev', netdev,
                '-device', f'{profile.nic_model},netdev=net0'
            ])

...
class FirmwareExtractor:
    ...
    def analyze(self, firmware_path: str) -> Dict[str, Any]:
        ...
        for signature, (sig_type, description) in self.SIGNATURES.items():
            ...
            while True:
                ...
                if 'squashfs' in sig_type:
                    ...
                    result['endianness'] = Endianness.LITTLE if '_le' in sig_type else Endianness.BIG
        ...
        logger.info(f"Analysis complete: {result['architecture'].value}, {result['filesystem_type'].value}")
    ...
    def extract(self, firmware_path: str, output_dir: str) -> Path:
        ...
        for comp in analysis['components']:
            if comp['type'] in ['squashfs_le', 'squashfs_be', 'cramfs_le', 'cramfs_be', 'jffs2_le', 'jffs2_be']:
                fs_component = comp
                break
    ...
    def _manual_extract(
        ...
        subprocess.run([
            'dd', f'if={firmware_path}', f'of={temp_fs}',
            'bs=1', f'skip={offset}'
        ...
        if fs_type == FilesystemType.SQUASHFS:
            try:
                ...
                subprocess.run(
                    ['sasquatch', '-d', str(rootfs_dir), str(temp_fs)],
                    capture_output=True
                )
        
        elif fs_type == FilesystemType.CRAMFS:
            subprocess.run(
                ['cramfsck', '-x', str(rootfs_dir), str(temp_fs)],
                capture_output=True
            )
            ...
            subprocess.run(
                ['jefferson', str(temp_fs), '-d', str(rootfs_dir)],
                capture_output=True
            )
    ...
    def _analyze_entropy(self, data: bytes, block_size: int = 1024) -> Dict:
        ...
        entropies = [
            calc_entropy(data[i*block_size:(i+1)*block_size])

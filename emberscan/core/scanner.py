...
from .models import (
    FirmwareInfo, ScanSession, ScanResult, ScanStatus,
    Vulnerability, EmulationState, Architecture
)
from .exceptions import (
    EmberScanError, ExtractionError, EmulationError,
    ScannerError, ConfigurationError
)
...
class EmberScanner:
    ...
    def scan_firmware(
        ...
        try:
            ...
            logger.info(f"Scan completed - {summary['total_vulnerabilities']} vulnerabilities found")
    ...
    def _extract_firmware(self, firmware: FirmwareInfo) -> Path:
        ...
        rootfs_path = self.extractor.extract(
            firmware.file_path,
            str(extraction_dir)
        )
        ...
        with ThreadPoolExecutor(max_workers=self.config.scanner.parallel_scans) as executor:
                ...
                try:
                    ...
                    if scanner_name == 'web':
                        ...
                        future = executor.submit(
                            scanner.scan, target_ip, firmware,
            ...
            for future in as_completed(futures, timeout=self.config.scanner.timeout_per_scan):
                ...
                try:
                    ...
                    results.append(result)
                    logger.info(f"{scanner_name} scanner completed: {len(result.vulnerabilities)} findings")
                except Exception as e:

    ...
    def generate(self, session: ScanSession, output_dir: str) -> str:
        ...
        report = {
            ...
            'scan_info': {
                ...
                'duration_seconds': (session.completed_at - session.started_at).total_seconds() if session.completed_at and session.started_at else None,
            },
        ...
        for vuln in session.all_vulnerabilities:
            ...
            if rule_id not in rules:
                rules[rule_id] = {
                    ...
                    'shortDescription': {
                        'text': f'{vuln.vuln_type.value} vulnerability'
                    },
